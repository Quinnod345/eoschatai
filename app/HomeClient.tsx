...
export default function HomeClient() {
  ...
  // Initialize all elements with opacity 0 to prevent flash
  useEffect(() => {
    ...
    // Ensure blur-text elements are visible
    gsap.set('.blur-text', { opacity: 1 });
    
    // Set other elements that don't use blur-text class
    gsap.set([
      '.feature-card:not(.blur-text)',
      '.stat-item:not(.blur-text)',
      '.step-item:not(.blur-text)',
      '.testimonial-card:not(.blur-text)',
      '.benefit-card:not(.blur-text)',
      '.integration-item:not(.blur-text)'
    ], { opacity: 0 });
  }, [showEntrance]);
  ...
  useGSAP(
    () => {
      if (showEntrance) return; // Skip animations while entrance is showing
      ...
      // Remove complex noise overlay

      // Remove all hero animations - everything visible immediately
      gsap.set([
        '.hero-title',
        '.hero-subtitle', 
        '.hero-buttons',
        '.hero-image',
        '.floating-element'
      ], { opacity: 1, visibility: 'visible', clearProps: 'all' });

      // Simple section titles animation - no blur
      gsap.utils.toArray('.section-title').forEach((title: any) => {
        gsap.set(title, { opacity: 1 }); // Always visible
      ...
      // Feature cards with stagger and depth
      ...
      // Enhanced stats animation with blur
      gsap.utils.toArray('.stat-item').forEach((stat: any, index) => {
        gsap.set(stat, { opacity: 1 });
        
        const numberElement = stat.querySelector('.stat-number');
        const finalValue = Number.parseInt(numberElement.getAttribute('data-value'));
      ...
      // Remove parallax effects - keep simple

      // Remove blur animations - keep text always visible
      gsap.utils.toArray('.blur-text').forEach((text: any) => {
      ...
      gsap.utils.toArray('.benefit-card').forEach((card: any, index) => {
        gsap.set(card, { opacity: 1 });
      ...
      // Simple integration items animation
      gsap.utils.toArray('.integration-item').forEach((item: any, index) => {
    ...
    { scope: containerRef, dependencies: [showEntrance] }
  );
  ...
  return (
    <>
            ...
            <div className="container mx-auto px-4 md:px-6 relative z-10">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
                <div className="stat-item text-center glass-morphism rounded-xl p-6 shadow-lg">
                  <div className="stat-number text-3xl md:text-4xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-eos-orange to-eos-orange/70 mb-2" data-value="500">
                    0
                  </div>
                  <p className="text-sm text-muted-foreground">Companies Using EOS AI</p>
                </div>
                <div className="stat-item text-center glass-morphism rounded-xl p-6 shadow-lg">
                  <div className="stat-number text-3xl md:text-4xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-eos-orange to-eos-orange/70 mb-2" data-value="10000">
                    0
                  </div>
                  <p className="text-sm text-muted-foreground">Hours Saved Monthly</p>
                </div>
                <div className="stat-item text-center glass-morphism rounded-xl p-6 shadow-lg">
                  <div className="stat-number text-3xl md:text-4xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-eos-orange to-eos-orange/70 mb-2" data-value="95">
                    0
                  </div>
                  <p className="text-sm text-muted-foreground">% User Satisfaction</p>
                </div>
                <div className="stat-item text-center glass-morphism rounded-xl p-6 shadow-lg">
                  <div className="stat-number text-3xl md:text-4xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-eos-orange to-eos-orange/70 mb-2" data-value="247">
                    0
                  </div>
      ...
      <style jsx global>{`
        ...
        /* Non-blur-text animated elements */
        .feature-card:not(.blur-text),
        .stat-item:not(.blur-text),
        .step-item:not(.blur-text),
        .testimonial-card:not(.blur-text),
        .benefit-card:not(.blur-text),
        .integration-item:not(.blur-text),
        .section-title:not(.blur-text),
        .section-subtitle:not(.blur-text) {
          opacity: 0;
        }
