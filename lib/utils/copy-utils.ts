...
async function loadDependencies() {
  if (!unified) {
    const [unifiedModule, remarkParseModule, remarkGfmModule, remarkRehypeModule, rehypeStringifyModule] = await Promise.all([
      import('unified'),
      import('remark-parse'),
      import('remark-gfm'),
...
export async function markdownToHtml(markdown: string): Promise<string> {
  ...
  // Style code blocks
  html = html.replace(/<pre>/g, '<pre style="background-color: #f6f8fa; padding: 16px; border-radius: 6px; overflow-x: auto; font-family: Monaco, Consolas, monospace; font-size: 13px;">');
  
  // Style inline code
  html = html.replace(/<code>/g, '<code style="background-color: #f3f4f6; padding: 2px 4px; border-radius: 3px; font-family: Monaco, Consolas, monospace; font-size: 85%;">');
  
  // Style tables
  html = html.replace(/<table>/g, '<table style="border-collapse: collapse; width: 100%; margin: 16px 0;">');
  html = html.replace(/<th>/g, '<th style="border: 1px solid #dfe2e5; padding: 8px; background-color: #f6f8fa; font-weight: 600;">');
  html = html.replace(/<td>/g, '<td style="border: 1px solid #dfe2e5; padding: 8px;">');
  
  // Style blockquotes
  html = html.replace(/<blockquote>/g, '<blockquote style="border-left: 4px solid #dfe2e5; margin: 16px 0; padding-left: 16px; color: #6a737d;">');
  
  // Style links
  html = html.replace(/<a /g, '<a style="color: #0366d6; text-decoration: none;" ');
  
  // Style headings
  html = html.replace(/<h1>/g, '<h1 style="font-size: 2em; font-weight: 600; margin: 0.67em 0;">');
  html = html.replace(/<h2>/g, '<h2 style="font-size: 1.5em; font-weight: 600; margin: 0.83em 0;">');
  html = html.replace(/<h3>/g, '<h3 style="font-size: 1.17em; font-weight: 600; margin: 1em 0;">');
  
  // Style lists
  html = html.replace(/<ul>/g, '<ul style="margin: 16px 0; padding-left: 2em;">');
  html = html.replace(/<ol>/g, '<ol style="margin: 16px 0; padding-left: 2em;">');
