...
export const bookmarkedChat = pgTable(
  ...
  (table) => {
    return {
      bookmarkedUserChatIdx: uniqueIndex('bookmarked_user_chat_idx').using(
        'btree',
        table.userId.asc().nullsLast(),
        table.chatId.asc().nullsLast(),
      ),
      bookmarkedUserIdx: index('bookmarked_user_idx').using(
        'btree',
        table.userId.asc().nullsLast(),
      ),
...
export const googleCalendarToken = pgTable(
  ...
  (table) => {
    return {
      ...
      googleCalendarTokenUserIdUnique: unique(
        'GoogleCalendarToken_userId_unique',
      ).on(table.userId),
    };
  },
);
...
export const documentRevision = pgTable(
  ...
  (table) => {
    return {
      documentRevisionDocIdx: index('document_revision_doc_idx').using(
        'btree',
        table.documentId.asc().nullsLast(),
      ),
      documentRevisionDocTimeIdx: index('document_revision_doc_time_idx').using(
        'btree',
        table.documentId.asc().nullsLast(),
...
export const personaComposerDocument = pgTable(
  ...
  (table) => {
    return {
      personaComposerDocUnique: uniqueIndex(
        'persona_composer_doc_unique',
      ).using(
        'btree',
        table.personaId.asc().nullsLast(),
        table.documentId.asc().nullsLast(),
      ),
...
export const l10RecordingContext = pgTable(
  'L10RecordingContext',
  {
    id: uuid().defaultRandom().primaryKey().notNull(),
...
export const personaDocument = pgTable(
  ...
  (table) => {
    return {
      ...
      personaDocumentPersonaIdDocumentIdUnique: unique(
        'PersonaDocument_personaId_documentId_unique',
      ).on(table.personaId, table.documentId),
    };
  },
);
...
export const pinnedMessage = pgTable(
  ...
  (table) => {
    return {
      pinnedChatIdx: index('pinned_chat_idx').using(
        'btree',
        table.chatId.asc().nullsLast(),
      ),
      pinnedUserMessageIdx: index('pinned_user_message_idx').using(
        'btree',
        table.userId.asc().nullsLast(),
...
export const profileDocument = pgTable(
  ...
  (table) => {
    return {
      ...
      profileDocumentProfileIdDocumentIdUnique: unique(
        'ProfileDocument_profileId_documentId_unique',
      ).on(table.profileId, table.documentId),
    };
  },
);

export const personaProfile = pgTable(
  'PersonaProfile',
  {
    id: uuid().defaultRandom().primaryKey().notNull(),
    personaId: uuid().notNull(),
...
export const bundleDocument = pgTable(
  ...
  (table) => {
    return {
      bundleUserDocUnique: uniqueIndex('bundle_user_doc_unique').using(
        'btree',
        table.userId.asc().nullsLast(),
        table.bundleId.asc().nullsLast(),
        table.documentId.asc().nullsLast(),
      ),
...
export const systemEmbeddings = pgTable(
  ...
  (table) => {
    return {
      namespaceIdx: index('namespace_idx').using(
        'btree',
        table.namespace.asc().nullsLast(),
      ),
...
export const voiceTranscript = pgTable(
  'VoiceTranscript',
  {
    id: uuid().defaultRandom().primaryKey().notNull(),
    recordingId: uuid().notNull(),
...
export const messageEditHistory = pgTable(
  ...
  (table) => {
    return {
      editHistoryMessageIdx: index('edit_history_message_idx').using(
        'btree',
        table.messageId.asc().nullsLast(),
      ),
      editHistoryTimeIdx: index('edit_history_time_idx').using(
        'btree',
        table.editedAt.asc().nullsLast(),
      ),
      editHistoryUserIdx: index('edit_history_user_idx').using(
        'btree',
        table.editedBy.asc().nullsLast(),
      ),
...
export const nexusResearchSession = pgTable(
...
export const nexusResearchEmbedding = pgTable(
  ...
  (table) => {
    return {
      ...
      researchSessionEmbeddingIdx: index(
        'research_session_embedding_idx',
      ).using('btree', table.sessionId.asc().nullsLast()),
...
export const nexusResearchResult = pgTable(
  ...
  (table) => {
    return {
      researchResultSessionIdx: index('research_result_session_idx').using(
        'btree',
        table.sessionId.asc().nullsLast(),
      ),
      researchResultUrlIdx: index('research_result_url_idx').using(
        'btree',
...
export const nexusResearchReport = pgTable(
  ...
  (table) => {
    return {
      researchSessionReportIdx: uniqueIndex(
        'research_session_report_idx',
      ).using(
        'btree',
        table.sessionId.asc().nullsLast(),
        table.reportType.asc().nullsLast(),
      ),
...
export const voiceRecording = pgTable(
  'VoiceRecording',
  {
    id: uuid().defaultRandom().primaryKey().notNull(),
    userId: uuid().notNull(),
...
export const feedback = pgTable(
  ...
  (table) => {
    return {
      feedbackChatIdx: index('feedback_chat_idx').using(
        'btree',
        table.chatId.asc().nullsLast(),
      ),
      feedbackUserMessageIdx: index('feedback_user_message_idx').using(
        'btree',
        table.userId.asc().nullsLast(),
        table.messageId.asc().nullsLast(),
      ),
...
export const l10Meeting = pgTable(
  'L10Meeting',
  {
    id: uuid().defaultRandom().primaryKey().notNull(),
    userId: uuid().notNull(),
  ...
  (table) => {
    return {
      l10MeetingComposerIdx: index('l10_meeting_composer_idx').using(
        'btree',
        table.composerId.asc().nullsLast(),
      ),
      l10MeetingStatusIdx: index('l10_meeting_status_idx').using(
        'btree',
        table.status.asc().nullsLast(),
      ),
      l10MeetingUserIdx: index('l10_meeting_user_idx').using(
        'btree',
        table.userId.asc().nullsLast(),
      ),
...
export const l10AgendaItem = pgTable(
  'L10AgendaItem',
  {
    id: uuid().defaultRandom().primaryKey().notNull(),
    meetingId: uuid().notNull(),
  ...
  (table) => {
    return {
      l10AgendaMeetingIdx: index('l10_agenda_meeting_idx').using(
        'btree',
...
export const userDocumentEmbedding = pgTable(
  ...
  (table) => {
    return {
      userDocUserIdIdx: index('user_doc_user_id_idx').using(
        'btree',
        table.userId.asc().nullsLast(),
      ),
      userEmbeddingIdx: index('user_embedding_idx').using(
        'hnsw',
        table.embedding.asc().nullsLast().op('vector_cosine_ops'),
      ),
...
export const l10Issue = pgTable(
  ...
  (table) => {
    return {
      l10IssueMeetingIdx: index('l10_issue_meeting_idx').using(
        'btree',
        table.meetingId.asc().nullsLast(),
      ),
...
export const l10Todo = pgTable(
  ...
  (table) => {
    return {
      l10TodoMeetingIdx: index('l10_todo_meeting_idx').using(
        'btree',
        table.meetingId.asc().nullsLast(),
      ),
...
export const documentVersion = pgTable(
  ...
  (table) => {
    return {
      documentVersionCreatedIdx: index('document_version_created_idx').using(
        'btree',
        table.createdAt.asc().nullsLast(),
      ),
      documentVersionDocumentIdx: index('document_version_document_idx').using(
        'btree',
        table.documentId.asc().nullsLast(),
      ),
      documentVersionUnique: uniqueIndex('document_version_unique').using(
        'btree',
        table.documentId.asc().nullsLast(),
        table.version.asc().nullsLast(),
      ),
      documentVersionUserIdx: index('document_version_user_idx').using(
        'btree',
...
export const documentEditOperation = pgTable(
  ...
  (table) => {
    return {
      documentEditDocumentIdx: index('document_edit_document_idx').using(
        'btree',
        table.documentId.asc().nullsLast(),
      ),
      documentEditSequenceIdx: index('document_edit_sequence_idx').using(
        'btree',
        table.documentId.asc().nullsLast(),
        table.sequenceNumber.asc().nullsLast(),
      ),
      documentEditSequenceUnique: uniqueIndex(
        'document_edit_sequence_unique',
      ).using(
        'btree',
        table.documentId.asc().nullsLast(),
        table.sequenceNumber.asc().nullsLast(),
      ),
      documentEditTimestampIdx: index('document_edit_timestamp_idx').using(
        'btree',
        table.timestamp.asc().nullsLast(),
      ),
      documentEditUserIdx: index('document_edit_user_idx').using(
        'btree',
        table.userId.asc().nullsLast(),
      ),
...
export const documentHistoryPointer = pgTable(
  ...
  (table) => {
    return {
      documentHistoryPointerUnique: uniqueIndex(
        'document_history_pointer_unique',
      ).using(
        'btree',
        table.documentId.asc().nullsLast(),
        table.userId.asc().nullsLast(),
...
export const userMemoryEmbedding = pgTable(
  ...
  (table) => {
    return {
      ...
      userMemoryIdIdx: index('user_memory_id_idx').using(
        'btree',
...
export const documentHistory = pgTable(
  ...
  (table) => {
    return {
      docHistoryDocumentIdx: index('doc_history_document_idx').using(
        'btree',
        table.documentId.asc().nullsLast(),
      ),
      docHistoryTimestampIdx: index('doc_history_timestamp_idx').using(
        'btree',
        table.timestamp.asc().nullsLast(),
      ),
      docHistoryUserIdx: index('doc_history_user_idx').using(
        'btree',
        table.userId.asc().nullsLast(),
      ),
...
export const documentEditSession = pgTable(
  ...
  (table) => {
    return {
      editSessionActiveIdx: index('edit_session_active_idx').using(
        'btree',
        table.isActive.asc().nullsLast(),
      ),
      editSessionDocUserIdx: index('edit_session_doc_user_idx').using(
        'btree',
        table.documentId.asc().nullsLast(),
        table.userId.asc().nullsLast(),
      ),
...
export const documentUndoStack = pgTable(
  ...
  (table) => {
    return {
      undoStackDocUserIdx: uniqueIndex('undo_stack_doc_user_idx').using(
        'btree',
        table.documentId.asc().nullsLast(),
        table.userId.asc().nullsLast(),
      ),
